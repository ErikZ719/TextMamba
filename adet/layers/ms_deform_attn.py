...
class MSDeformAttn(nn.Module):
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index,
        ...
        mask1 = torch.zeros(N, self.n_heads, self.n_levels * self.n_points, Len_q, requires_grad=False).cuda()
        index = torch.topk(attention_weights, k=int(Len_q*9/10), dim=-1, largest=True)[1]
        mask1.scatter_(-1, index, 1.)
        attention_weights = torch.where(mask1 > 0, attention_weights, torch.full_like(attention_weights, float('-1e9')))
