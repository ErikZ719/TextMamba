...
class DeformableTransformer_Det(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed, bk_feature=None):
        ...
        query_embed = tgt_undetach.detach()[:, :, None] + query_embed.unsqueeze(0).expand(bs, -1, -1,
                                                                                          -1)  # (bs, nq,1, 256)
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(
        ...
        # ffn
        self.linear1_1 = nn.Linear(d_model, d_ffn)
        self.linear1_2 = nn.Linear(d_model, int(d_ffn * 2))
        self.activation = _get_activation_fn(activation)
        self.dropout2 = nn.Dropout(dropout)
        self.dropout2_1 = nn.Dropout(dropout)
        self.linear2_1 = nn.Linear(d_ffn, d_model)
        self.linear2_2 = nn.Linear(int(d_ffn * 2), d_model)
    ...
    def forward_ffn(self, src):
        src1 = self.linear2_1(self.dropout2(self.activation(self.linear1_1(src))))
        src2 = self.linear2_2(self.dropout2_1(self.activation(self.linear1_2(src))))

        src = src + self.dropout3(src1) + self.dropout3_2(src2)
        src = self.norm2(src)
        return src
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, padding_mask=None):
        ...
        src_ss2d = self.SS2D(src2)
        src = self.dropout1(src2) + self.dropout1(src_ss2d)
...
class DeformableTransformerDecoder_AnchorDet(nn.Module):
    ...
    def forward_prediction_reg_and_seg(self, output, mask_features_ori, query_pos, semantic_mask, last_reference_points,
                                       level_embed, lvl):
        ...
        for i in range(1, 4):
            ...
            ROI = F.softmax(lower_level_mask.unsqueeze(2).flatten(-2), dim=1).detach() * \
                  F.interpolate(semantic_mask, size=lower_level_mask.shape[-2:], mode="nearest").unsqueeze(2).flatten(
                      -2)
        ...
        outputs_anchor_weights_map = F.softmax(outputs_mask.reshape(b, nq, h * w) / 0.3, dim=-1).reshape(b, nq, h,
                                                                                                         w).unsqueeze(
            -1)
